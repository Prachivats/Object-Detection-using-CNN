...
class cnn_network:
    def __init__(self):
        ...
        self.model_layers = []
        #L: 0
        self.model_layers.append(Convolution_Layer(inputs_channel=3, num_filters=6,
                                                   kernel_size=5, padding=2, stride=1, learning_rate=Learning_Rate, name='conv1'))
        # #Ly: 1
        self.model_layers.append(ReLu_Activation())
        # #Ly: 2
        self.model_layers.append(Maxpool_Layer(
            pool_size=2, stride=2, name='maxpool2'))
        # #Ly: 3
        self.model_layers.append(Convolution_Layer(inputs_channel=6, num_filters=16,
                                                   kernel_size=5, padding=2, stride=1, learning_rate=Learning_Rate, name='conv3'))
        # #Ly: 4
        self.model_layers.append(ReLu_Activation())
        # #Ly: 5
        self.model_layers.append(Maxpool_Layer(
        ...
        # #Ly: 6
        self.model_layers.append(Convolution_Layer(inputs_channel=16, num_filters=72,
                                                   kernel_size=5, padding=2, stride=1, learning_rate=Learning_Rate, name='conv5'))
        # #Ly: 7
        self.model_layers.append(ReLu_Activation())
        # #Ly: 8
        self.model_layers.append(Flattening_Layer())
        # #Ly: 9
        self.model_layers.append(FullyConnected_Layer(
            num_inputs=45000, num_outputs=36, learning_rate=Learning_Rate, name='fc6'))
        # #Ly: 10
        self.model_layers.append(ReLu_Activation())
        # #Ly: 11
        self.model_layers.append(FullyConnected_Layer(
            num_inputs=36, num_outputs=6, learning_rate=Learning_Rate, name='fc7'))
        # #Ly: 12
    ...
    def training(self, training_data, training_label, batch_size, epoch):
        model_loss = 0
        for e in range(epoch):
            for batch_index in range(0, training_data.shape[0], batch_size):
                ...
                for lay in range(self.layers_numb):
                    ...
                    output = self.model_layers[lay].forward_propagation(
                        train_data)
                    train_data = output
                    print(output.shape)
                model_loss += cross_entropy_loss(train_data, train_labels)
                print(model_loss)
                dy = train_labels
